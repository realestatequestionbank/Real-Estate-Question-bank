#!/usr/bin/env python3
"""
Multi-State HazMat Scraper for DMV Written Test
Scrapes HazMat CDL questions from multiple states to build a large deduplicated
hazmat question bank (standardized federally).
"""
...
def main():
    ...
    all_questions = []
    seen_ids = set()
    seen_questions = set() # Check text duplicates too, just in case IDs differ across states
    
    output_file = "california_cdl_hazmat_questions.csv"
    delay = 1.2
    
    print("Starting Multi-State HazMat Consolidation...")
    ...
    for state in STATES:
        ...
        for test_num in range(1, 6): # Up to 5 tests per state
            for page_num in range(1, 6): # Up to 5 pages per test
                url = f"https://www.dmv-written-test.com/{state}/cdl/hazmat-{test_num}.html?page={page_num}"
    ...
    if all_questions:
        print(f"\nSuccessfully gathered {len(all_questions)} unique HazMat questions.")
