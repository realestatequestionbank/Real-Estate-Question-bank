...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `FREE ${data.stateName} CDL ${category.name} Practice Test (2026)`
  const description = `Pass your ${data.stateName} CDL ${category.name} written exam. Free practice questions and detailed explanations based on the official ${data.stateName} ${data.departmentName} commercial driver's manual.`
  const url = `https://www.realestatequestionbank.com/${stateKey}-${params.test}-cdl-permit-test`
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${data.stateName} CDL ${category.name} Practice Test`
        }
      ]
    },
    twitter: {
...
function loadQuestions(state: string, categoryId: string, categoryName: string): Question[] {
  try {
    ...
    let filename = `${state}_cdl_${filePrefix}_questions.csv`
    const csvPath = path.join(process.cwd(), 'public', 'data', filename)
    
    // Fallback to California if specific state file doesn't exist
    let finalCsvText = ''
    if (fs.existsSync(csvPath)) {
      finalCsvText = fs.readFileSync(csvPath, 'utf-8')
    } else {
      const fallbackFilename = `california_cdl_${filePrefix}_questions.csv`
    ...
    console.error(`Failed to load CDL questions for ${state} - ${categoryId}:`, error)
    return []
  }
}

export default function CdlTestSpecificPageWrapper({ params }: PageProps) {
  ...
  // Load 25 questions server-side for SEO
