#!/usr/bin/env python3
"""
Multi-State CDL Scraper for DMV Written Test
Scrapes CDL questions for a specific category from multiple states to build a large deduplicated
question bank (federally standardized).
"""
...
def main():
    parser = argparse.ArgumentParser(description="Multi-State CDL Scraper")
    parser.add_argument(
        "--type",
        required=True,
        choices=["air-brakes", "combination", "double", "tank"],
        help="Shorthand category name (e.g. air-brakes, combination, double, tank)"
    )
    parser.add_argument(
        "--output",
        default=None,
    ...
    print(f"Starting Multi-State Consolidation for category: {test_type}...")
    ...
    for state in STATES:
        ...
        for test_num in range(1, 6): # Up to 5 tests per state
            for page_num in range(1, 6): # Up to 5 pages per test
                url = f"https://www.dmv-written-test.com/{state}/cdl/{test_type}-{test_num}.html?page={page_num}"
    ...
    if all_questions:
        print(f"\nSuccessfully gathered {len(all_questions)} unique questions.")
