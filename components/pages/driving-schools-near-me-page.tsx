...
interface DrivingSchoolLocation {
  id: string
  name: string
  address: string
  lat: number
...
export function DrivingSchoolsNearMePageContent() {
  const [zipCode, setZipCode] = useState('')
  const [locations, setLocations] = useState<DrivingSchoolLocation[]>([])
  ...
  const searchByZipCode = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'Failed to search for driving schools')
        setLocations([])
        return
      }
  ...
  const getDirectionsUrl = (location: DrivingSchoolLocation) => {
    const address = encodeURIComponent(location.address)
    return `https://www.google.com/maps/dir/?api=1&destination=${address}`
  }

  const getGoogleMapsUrl = (location: DrivingSchoolLocation) => {
  ...
  const getMapUrl = useCallback(() => {
    ...
    const markers = locations.slice(0, 10).map((loc, index) => {
      ...
      return `markers=color:green%7Clabel:${label}%7C${loc.lat},${loc.lng}`
    }).join('&')
      ...
      <main>
        ...
        <div className="relative pt-20 pb-12 md:pb-16 overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-50 via-white to-blue-50">
            <div className="absolute top-20 left-20 w-72 h-72 bg-emerald-100/30 rounded-full blur-3xl animate-pulse"></div>
            <div className="absolute bottom-20 right-20 w-96 h-96 bg-blue-100/30 rounded-full blur-3xl animate-pulse delay-700"></div>
          </div>

          <div className="container mx-auto px-4 relative z-10">
            <div className="text-center max-w-4xl mx-auto">
              {/* Badge */}
              <div className="inline-flex items-center gap-2 bg-emerald-500/5 backdrop-blur-sm border border-emerald-500/20 rounded-full px-4 py-2 mb-8 animate-fade-in">
                <Car className="w-4 h-4 text-emerald-600" />
                <span className="text-sm font-medium text-gray-700">Driving School Locator</span>
              </div>

              <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-6 tracking-tight leading-tight">
                Find a
                <span className="text-emerald-600"> Driving School Near You</span>
              </h1>

              <p className="text-lg md:text-xl text-gray-600 mb-10 max-w-2xl mx-auto leading-relaxed">
                Search by zip code or use your current location to find nearby driving schools
                with ratings, hours, phone numbers, and directions.
              </p>
                  ...
                  <div className="flex flex-row gap-2 md:gap-4">
                    <div className="flex-1 relative">
                      ...
                      <Input
                        ...
                        className="pl-12 pr-4 py-6 text-lg border-2 border-gray-100 rounded-xl focus:border-emerald-500 focus:ring-emerald-500/20 focus-visible:ring-0 focus-visible:ring-offset-0"
                        maxLength={5}
                      />
                    </div>
                    <Button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 md:px-8 py-6 text-lg rounded-xl shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:hover:scale-100 flex-shrink-0"
                    >
                      {isLoading ? (
            ...
            <div className="max-w-6xl mx-auto">
              {locations.length > 0 ? (
                <>
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-bold text-gray-900">
                      {locations.length} Driving School{locations.length !== 1 ? 's' : ''} Found
                    </h2>
                  </div>
                    ...
                    <div className="order-2 lg:order-1">
                      ...
                      >
                        {process.env.NEXT_PUBLIC_GOOGLE_MAPS_KEY && searchLocation ? (
                          <img
                            src={getMapUrl() || ''}
                            alt="Map showing driving school locations"
                        ...
                        <div className="mt-4 p-4 bg-gray-50 rounded-xl">
                          ...
                          <div className="flex flex-wrap gap-3">
                            {locations.slice(0, 10).map((loc, index) => (
                              ...
                              >
                                <span className="w-5 h-5 bg-emerald-600 text-white text-xs font-bold rounded flex items-center justify-center">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between gap-3 mb-2">
                                  ...
                                  {location.distance !== undefined && (
                                    <span className="flex-shrink-0 px-2 py-1 bg-emerald-50 text-emerald-700 text-sm font-medium rounded-lg">
                                ...
                                {location.phone && (
                                  <a
                                    ...
                                    className="flex items-center gap-2 text-gray-600 hover:text-emerald-600 transition-colors mb-3"
                                  >
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-2">No Driving Schools Found</h3>
                  <p className="text-gray-600 max-w-md mx-auto">
                    We couldn't find any driving schools near that location.
        ...
        <div className="container mx-auto px-4 pb-16">
          <div className="max-w-4xl mx-auto">
            <div className="bg-gradient-to-br from-gray-50 to-white rounded-3xl p-8 md:p-12 border border-gray-100">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Tips for Choosing a Driving School</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-1">Check Licensing</h3>
                    <p className="text-gray-600 text-sm">Verify the school is licensed by your state's Real Estate.</p>
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-1">Read Reviews</h3>
                    <p className="text-gray-600 text-sm">Check Google reviews and ask for recommendations.</p>
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-1">Ask About Instructors</h3>
                    <p className="text-gray-600 text-sm">Certified instructors with experience make a difference.</p>
                  </div>
                </div>
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-1">Compare Packages</h3>
                    <p className="text-gray-600 text-sm">Look for packages that include behind-the-wheel training.</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row sm:justify-center gap-4">
                <Button
                  onClick={() => router.push('/real-estate-near-me')}
                  variant="outline"
                  className="w-full sm:w-auto px-8 py-4 text-base rounded-xl border-2 transition-all hover:bg-gray-50"
                >
                  Find Real Estate Near Me
                </Button>
                <Button
                  onClick={() => router.push('/')}
                  className="bg-emerald-600 hover:bg-emerald-700 text-white w-full sm:w-auto px-8 py-4 text-base rounded-xl shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95"
                >
                  Start Practice Test
                </Button>
              </div>
