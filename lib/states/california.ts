export const californiaStateData = {
  stateName: 'California',
  pricing: {
    freeQuestions: 20,
    premiumQuestions: 2000,
    plans: {
      sevenDay: { duration: 7, originalPrice: 20, discountedPrice: 9.99, badge: '50% OFF' },
      thirtyDay: { duration: 30, originalPrice: 40, discountedPrice: 19.99, badge: '50% OFF' },
      ninetyDay: { duration: 90, originalPrice: 69.99, discountedPrice: 34.99, badge: '50% OFF' }
    }
  },
  seo: {
    title: 'FREE California Real Estate Exam Practice 2026 | Real Estate Question Bank',
    description: 'Pass your California Real Estate Exam on the 1st try. Includes 2,000+ realistic practice questions, mock tests, and explanations. Updated for 2026!',
    keywords: [
      'California real estate exam',
      'California real estate practice test',
      'California real estate license exam prep',
      'California Department of Real Estate (DRE) licensing test',
      'how to pass real estate exam in California'
    ]
  },
  testOverview: {
    totalQuestions: 150,
    passingScore: 105,
    timeLimit: '3.25 Hours',
    ageRequirements: {
      under18: 'Not eligible (must be 18 years old or older)',
      over18: 'Eligible immediately after completing 135 hours of pre-licensing education.'
    },
    topicsCovered: [
      'Real Estate Principles and Practices',
      'Real Estate Law and Contracts',
      'Agency Relationships and Disclosures',
      'Property Valuation and Financial Math',
      'Financing, Mortgages, and Escrow',
      'State-Specific Licensing Laws and Commission Rules'
    ]
  },
  testFormat: {
    questionTypes: 'Multiple choice',
    correctAnswersNeeded: 105,
    incorrectAnswersAllowed: 45,
    retakePolicy: 'Can schedule a retake of the failed portion immediately'
  },
  commonMistakes: [
    {
      topic: 'Real Estate Math',
      description: 'Struggling with commission splits, tax prorations, and interest rate calculations.'
    },
    {
      topic: 'Agency Disclosures',
      description: 'Confusing single agency vs transaction brokerage rules.'
    },
    {
      topic: 'Contract Validity',
      description: 'Misunderstanding the difference between void and voidable contracts.'
    }
  ],
  studyTips: [
    {
      title: 'Practice Real Estate Math Formulas',
      description: 'Memorize critical formulas for commission, interest, LTV, and capitalization rates.'
    },
    {
      title: 'Study Key Definitions',
      description: 'Review escrow rules, fiduciary responsibilities, and licensing regulations.'
    },
    {
      title: 'Take Multiple Practice Exams',
      description: 'Practice consistently using our state-specific questions until scoring above 90%.'
    }
  ],
  faq: [
    {
      question: 'How many questions are on the California Real Estate Exam, and what is the passing score?',
      answer: 'The California salesperson licensing exam consists of 150 multiple-choice questions in total. You are given exactly 3 hours and 15 minutes (3.25 hours) to complete the test. To pass the exam and qualify for your license, you must score at least 70%—which translates to answering at least 105 questions correctly. There is no penalty for guessing, so it is highly recommended to answer every question on the test.'
    },
    {
      question: 'How hard is the California Real Estate Licensing Exam?',
      answer: 'The California DRE salesperson exam is widely considered challenging, with a historical first-time pass rate hovering around 50% to 60%. The test checks your knowledge across diverse areas including complex real estate laws, fiduciary duties, property ownership types, finance, math, and professional standards. However, students who utilize our comprehensive question bank, practice simulators, and detailed feedback explanations see their pass probability increase to over 98%.'
    },
    {
      question: 'How many pre-licensing education hours are required in California?',
      answer: 'To qualify for the California Real Estate Salesperson exam, the Department of Real Estate (DRE) requires you to complete 135 hours of college-level pre-licensing coursework. This is divided into three mandatory 45-hour courses: Real Estate Principles, Real Estate Practice, and one elective course (such as Real Estate Finance, Legal Aspects of Real Estate, or Property Management). All courses must be completed through an approved education provider before you can apply to take the exam.'
    },
    {
      question: 'Can I retake the California Real Estate Exam if I fail?',
      answer: 'Yes, if you do not pass the exam on your first attempt, you can apply to retake it as many times as needed. There is no limit on the number of retakes. You will need to submit a re-examination application and pay the required fee again. While you can re-apply immediately, scheduling availability depends on current DRE examinee volumes, which can sometimes lead to wait times of several weeks between attempts, making it crucial to pass on your first try.'
    },
    {
      question: 'What topics are covered on the California Real Estate Exam?',
      answer: 'The exam covers seven major content areas defined by the California DRE: 1) Practice of Real Estate and Disclosures (approx. 24%), 2) Laws of Agency and Fiduciary Duties (approx. 12%), 3) Property Ownership and Land Use Controls (approx. 15%), 4) Valuation and Financial Analysis (approx. 14%), 5) Contracts (approx. 12%), 6) Financing (approx. 9%), and 7) Transfer of Property (approx. 8%). Fiduciary duties and disclosures represent the largest share of the exam.'
    },
    {
      question: 'What documents or items do I need to bring to the exam center?',
      answer: 'On your test day, you must present a valid, government-issued photo identification (such as a driver\'s license or passport) that matches the name on your exam application. You should also bring your official exam confirmation letter. Electronic devices, smartwatches, study materials, and personal belongings are strictly prohibited inside the testing room. The testing center will provide scratch paper, pencils, and a basic calculator for real estate math questions.'
    },
    {
      question: 'What is the difference between a Real Estate Salesperson and a Broker in California?',
      answer: 'A Real Estate Salesperson is licensed to perform real estate transactions under the supervision of a licensed broker. A Salesperson cannot operate independently. A Real Estate Broker, on the other hand, can run their own independent firm, hire sales agents, and manage transactions directly. To become a broker in California, you must have at least two years of full-time active salesperson experience within the last five years, complete 8 college-level courses, and pass a longer, more rigorous 200-question broker exam.'
    },
    {
      question: 'How does the 100% Pass Guarantee work?',
      answer: 'We are so confident in our study system that we offer a 100% money-back guarantee. To qualify, you must unlock and complete all practice questions in our Premium package. If you do this and still do not pass your official California DRE salesperson licensing exam, simply email us your official exam failure notification from the DRE within 30 days of your test. We will issue a full refund of your purchase price within 5-7 business days, with no questions asked.'
    }
  ],
  hasBlogs: false,
  relatedBlogs: [],
  lastUpdated: 'July 2026'
};
