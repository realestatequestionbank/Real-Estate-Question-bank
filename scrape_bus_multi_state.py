#!/usr/bin/env python3
"""
Multi-State School Bus Scraper for DMV Written Test
Scrapes School Bus CDL questions from multiple states to build a large deduplicated
school bus question bank (federally standardized).
"""
...
def main():
    ...
    output_file = "california_cdl_bus_questions.csv"
    delay = 1.2
    
    print("Starting Multi-State School Bus Consolidation...")
    ...
    for state in STATES:
        ...
        for test_num in range(1, 6): # Up to 5 tests per state
            for page_num in range(1, 6): # Up to 5 pages per test
                url = f"https://www.dmv-written-test.com/{state}/cdl/bus-{test_num}.html?page={page_num}"
    ...
    if all_questions:
        print(f"\nSuccessfully gathered {len(all_questions)} unique School Bus questions.")
